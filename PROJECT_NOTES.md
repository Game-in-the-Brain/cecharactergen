...
- [ ] **careers.json** — **ALL careers in ONE file**
  - Single comprehensive JSON file with metadata header describing all careers
  - M2 starts with 3 complete careers (Drifter, Marine, Scout)
  - M3 adds remaining 21 careers to same file
  - **Structure:**
    ```json
    {
      "_metadata": {
        "version": "1.0",
        "description": "CE Core Rulebook - All 24 careers",
        "totalCareers": 24,
        "careersList": [
      ...
      "drifter": { "id": "drifter", "enabled": true, ... },
      "marine": { "id": "marine", "enabled": true, ... },
      "scout": { "id": "scout", "enabled": true, ... },
      ... (21 more)
    }
    ```
  - Each career has: `id`, `name`, `qualification`, `survival`, `advancement`, `skills`, `ranks`, `mustering`, `events`, `mishaps`, `enabled` (boolean)
  - **Active/Inactive:** `enabled` field controls which careers appear in generation
  - **Career Management UI:** GM toggles careers on/off without editing JSON directly
...
**JSON Schema Example (careers.json — ONE file with ALL careers):**
```json
{
  "_metadata": {
    "version": "1.0",
    "description": "Cepheus Engine Core Rulebook Careers",
    "source": "CE SRD Chapter 2",
    "totalCareers": 24,
    "activeCareers": 22,
    "lastUpdated": "2026-03-03",
    "careersList": [
      {"id": "drifter", "name": "Drifter", "category": "Civilian", "description": "Wanderers without fixed home", "enabled": true},
      {"id": "marine", "name": "Marine", "category": "Military", "description": "Elite military forces", "enabled": true},
      {"id": "scout", "name": "Scout", "category": "Exploration", "description": "Explorers of unknown space", "enabled": true},
      {"id": "noble", "name": "Noble", "category": "Elite", "description": "Aristocracy and ruling class", "enabled": false, "disabledReason": "Not in frontier setting"},
      {"id": "pirate", "name": "Pirate", "category": "Criminal", "description": "Raiders and smugglers", "enabled": false}
    ]
  },
  "drifter": {
    "id": "drifter",
  ...
  "marine": { /* full career data */ },
  "scout": { /* full career data */ },
  "navy": { /* etc. for all 24 careers */ }
}
```

**Key Points:**
- ONE file contains ALL 24 careers
- `_metadata` section at top describes file contents
- Each career has `enabled` boolean (active/inactive)
- Career Management UI toggles these flags
- All career data in one place (events, mishaps, skills, etc.)
